...
keywords: REST API fundamentals, ONTAP tools Manager, ONTAP tools IP address, authentication token, asynchronous requests, HTTP methods, JSON, ONTAP tools for VMware vSphere
...
ONTAP tools for VMware vSphere 10 exposes a single external IP address (formerly load balancer) that provides a gateway to the capabilities of the virtual machine. You provide this address during initial configuration. The address is used by the ONTAP tools Manager interface and to access the Swagger documentation page and REST API directly.
