...
.. Click on ellipsis  menu, and click on *Reset configuration*
.. Click on ellipsis  menu and select *Delete*
+
...
After you migrate from old releases of ONTAP tools for VMware vSphere to 10.3 release, rescan the SRA adapters to verify that the details are updated on the VMware Live Site Recovery Storage Replication Adapters page.
...
+
Under *VM storage policy compliance* you can see the status of the compliance. It shows as *Out of date*
. Select the Storage VM policy and the corresponding VM
. Select *Apply*
+
When you check the compliance status under *VM storage policy compliance* now shows as compliant.  
