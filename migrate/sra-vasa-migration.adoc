...
summary: 'Migrate the VASA Provider from ONTAP tools for VMware vSphere 9.x to 10.6 and update the Storage Replication Adapter (SRA) for VMware Live Site Recovery.'
---
...
. Add the vCenter Server instance that you want to migrate to ONTAP tools for VMware vSphere 10.6. Refer to link:../configure/add-vcenter.html[Add a vCenter Server instance] for more information.
. Onboard the storage backend locally through the vCenter Server APIs for the ONTAP tools plug-in. Refer to link:../configure/add-storage-backend.html[Add a storage backend using the vSphere Client interface] for more information.
