...
|114 |Internal cert update in the mongo collection failed| In case of upgrade failure with error code ending with 114, please follow the below steps to recover/complete the upgrade.
Enable Diagnostic Shell
Execute 'sudo perl /home/maint/scripts/post-deploy-upgrade.pl --internal_cert_update_collection'
Please check the logs at /var/log/otvng/post-deploy-upgrade.log
//OTV10.5 P2 update
|===
