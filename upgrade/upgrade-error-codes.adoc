...
summary: "Use ONTAP tools upgrade error codes to identify failed workflows and troubleshoot upgrade issues in VMware vSphere."
---
...
|068 |Debian OS packages rollback has failed |Use zero-RPO or snapshot based recovery and retry upgrade.
...
|074 |Mount ISO has failed |Check the /var/log/otvng/upgrade-run.log and retry upgrade.
...
3. Check the logs at /var/log/otvng/post-deploy-upgrade.log
...
// OTVDOC-375 and OTVDOC-374 update
|094 |NTP configuration failed and the system rolled back to NTPsec a|
If the upgrade fails due to a failure in the Chrony installation:

. Check whether Chrony is installed:
+
----
systemctl status chrony
----
. If Chrony is installed and the issue is with the NTP servers you provided, check whether those servers are reachable:
+
----
...
----
+
If the servers are reachable, the issue is unrelated to server reachability. If a server is genuinely unreachable, remove it from ONTAP tools Manager and provide a reachable NTP server instead.
. If the upgrade fails due to any other issue during the NTP upgrade, check the following logs:
+
----
...
Check upgrade logs and log file at /var/log/otvng/cert_validation_error.log and retry the upgrade.
