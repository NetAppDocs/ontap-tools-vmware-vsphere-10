...
* In case of HA deployment, If the resource (CPU and memory) change option for the other two nodes is greyed out, then power off the nodes one by one and make the changes. Then power it on and wait for the services to come to a running state. If the resources are not greyed out, you can proceed to make the changes.
