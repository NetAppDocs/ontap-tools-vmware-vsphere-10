...
[.lead]
You can upgrade from ONTAP tools for VMware vSphere 10.4 or 10.5 to 10.6. However, to upgrade from ONTAP tools 10.0, 10.1, 10.2, or 10.3 to 10.6, you must first upgrade to 10.4 or 10.5 before proceeding to 10.6.
// 10.6 updates
[NOTE]
--
...
You can create a dedicated vCenter user and role with only the permissions required for OVF template deployment. For details, see the roles listed in https://review.docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10_otv105-releasebranch/concepts/rbac-vcenter-use.html#vsphere-object-hierarchy[Roles included with ONTAP tools for VMware vSphere 10].
+
