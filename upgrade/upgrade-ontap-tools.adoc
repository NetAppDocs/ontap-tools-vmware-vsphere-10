...
[.lead]
You can upgrade from ONTAP tools for VMware vSphere 10.3 or 10.4 to 10.5. However, direct upgrade from ONTAP tools 10.0, 10.1, or 10.2 to 10.5 is not supported. 

NOTE:

* In ASA r2 systems, you should upgrade to ONTAP tools for VMware vSphere 10.5 with ONTAP 9.16.1 before adding more storage availability zones (SAZs).
// https://jira.ngage.netapp.com/browse/OTVDOC-254 updates
* If the upgrade from ONTAP tools for VMware vSphere 10.3 or 10.4 to 10.5 release fails, rollback is not supported. To recover the setup, use RPO for ONTAP tools for VMware vSphere 10.2 and low RPO or snapshot recovery from ONTAP tools for VMware vSphere 10.3 onwards.

.Before you begin

Before you upgrade, ensure that the certificate on the ONTAP system is valid for more than 5 days. If the certificate is about to expire within the next 5 days, the upgrade fails.
