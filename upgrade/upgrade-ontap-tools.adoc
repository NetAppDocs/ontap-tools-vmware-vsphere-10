...
Before you upgrade, ensure that the certificate on the ONTAP system is valid for more than 5 days. If the certificate is about to expire within the next 5 days, the upgrade is not allowed.
...
If you're upgrading from ONTAP tools for VMware vSphere 10.2, 10.3 0r 10.4, you need to complete the following steps before proceeding with the upgrade task:
