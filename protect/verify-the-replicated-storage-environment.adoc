...
// OTVDOC-360 for P2
* If you are using custom SnapMirror policies, you must perform storage discovery from ONTAP tools at the following points before running any SRA workflow, otherwise SRA workflows will fail:
** After creating the custom policy on the protected site.
** After a reprotect operation, which creates the same policy on the recovery site. Perform discovery on both the protected and recovery sites.
