...
[NOTE]
When you add a new datastore, make sure to include it in the existing datastore group to ensure it is protected. Alternatively, you can add a new datastore and create a new volume or consistency group for protection. 
After adding a new datastore to a protected consistency group or volume, perform storage discovery and update SnapMirror on both the protected and recovery sites. You can run discovery manually or on a schedule to ensure the new datastore is detected and protected.
// Is it for all types of datastores? JIRA update: OTVDOC-285 and OTVDOC-286
