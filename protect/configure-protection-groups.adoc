...
summary: "Create protection groups for protected-site virtual machines and update storage discovery after adding datastores to protected volumes."
---
= Pair sites and configure protection groups in ONTAP tools
:icons: font
:imagesdir: ../media/
...
[NOTE]
Storage Replication Adapter (SRA) supports fan-out with one sync relationship of type Automated Failover Duplex and async relationship SnapMirror on consistency group. However, fan-out with two asynchronous SnapMirror relationships on consistency groups or fan-out SnapMirror relationships on volumes is not supported. Vault type SnapMirror relationships are not considered within these fan-out restrictions. Consistency group support in SRA is limited to SAN protocols (iSCSI and fibre channel) on ONTAP storage and ASA r2 storage systems. Consistency groups are not supported for NFS. If NFS datastore volumes are placed under a consistency group, the discover devices operation will fail.
// OTV10.5 P2 updates - Jani
// updated the note for OTVDOC-323 - jani

.Before you begin

* You should have VMware Live Site Recovery (VLSR) installed on the protected and recovery sites.
...
* Same version of VLSR installed
* Virtual machines
