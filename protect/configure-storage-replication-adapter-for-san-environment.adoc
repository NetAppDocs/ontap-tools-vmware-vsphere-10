...
. Verify that valid addresses such as the IP address or FQDN on which the NFS exports are present are specified in the *NFS Addresses* field when using the Array Manager wizard to add arrays to VMware Live Site Recovery. don't use the NFS hostname in the *NFS Addresses* field.
