...
. Verify that valid addresses such as the IPv4 or IPv6 address or FQDN on which the NFS exports are present are specified in the *NFS Addresses* field when using the Array Manager wizard to add arrays to VMware Live Site Recovery. Do not use the NFS hostname or FQDNin the *NFS Addresses* field.
