...
[NOTE]
Consistency groups (CGs) are supported in SRA workflows for SAN protocols (iSCSI and Fibre Channel) on both Unified and ASA r2 storage systems.
...
For iSCSI protocol configurations, ensure the following prerequisites are met on both the protected and recovery sites before running SRA workflows:

* The iSCSI storage adapter must be added on each ESXi host.
* The data LIF details must be configured on the iSCSI adapter. To complete this configuration, create a placeholder datastore of type iSCSI using the SVM on which the SRA workflows are configured. Refer to link:../protect/configure-placeholder-datastores.html[Configure placeholder datastores in ONTAP tools] for more details.
// OTVDOC-358 updates
