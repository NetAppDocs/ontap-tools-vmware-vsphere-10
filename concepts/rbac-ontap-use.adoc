...
summary: "Review ONTAP RBAC considerations for ONTAP tools before using it in a production environment."
---
= ONTAP RBAC in ONTAP tools for VMware vSphere
:icons: font
:imagesdir: ../media/

[.lead]
ONTAP tools for VMware vSphere supports role-based access control (RBAC) through ONTAP, enabling you to create custom users and roles that restrict access to storage resources. Understanding the configuration process and privilege levels helps you set up appropriate access controls before deploying ONTAP tools in a production environment.
...
The following section describes the major configuration steps at a high level. Refer to link:../configure/configure-user-role-and-privileges.html[Configure ONTAP user roles and privileges] for more details.
...
.. Scroll down to *Users and roles* and select `-->`.
...
The role privileges are arranged in four sets based on the level of access needed for ONTAP storage. The privileges that the roles are based on include:
...
After you've selected the configuration options for your environment, select *Add* and ONTAP creates the user and role. The name of the generated role is a concatenation of the following values:
...
The new user appears in the list on the *Users and roles* page. Note that both HTTP and ONTAPI user login methods are supported.
