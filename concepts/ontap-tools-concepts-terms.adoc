...
summary: "Review key concepts and terms used in ONTAP tools for VMware vSphere documentation."
---
= Key terms and definitions for ONTAP tools
:icons: font
:imagesdir: ../media/

[.lead]
Familiarize yourself with key concepts and terminology used in ONTAP tools for VMware vSphere. Definitions cover storage backends, virtual volumes (vVols), SnapMirror active sync, VASA, SRA, and other core components.
...
An NVMe subsystem can be associated with initiators, enabling them to access namespaces within the subsystem.
...
RPO measures how frequently you back up or replicate data. It specifies the exact point in time you need to restore data after an outage to resume business operations. For example, if an organization has an RPO of four hours, it can tolerate losing up to four hours of data in the event of a disaster.

=== SnapMirror active sync
SnapMirror active sync enables business services to continue operating even with a complete site failure, supporting applications to fail over transparently using a secondary copy. SnapMirror active sync does not require manual intervention or custom scripting to trigger a failover. Learn more about https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/index.html[SnapMirror active sync].
...
[NOTE]
SnapMirror active sync supports uniform host access for any deployment; only symmetric active/active deployments support non-uniform host access.
...
vVols provide a volume-level abstraction for storage used by a virtual machine. vVols include several benefits and provide an alternative to using a traditional LUN. A vVol datastore is typically associated with a single LUN that acts as a container for vVols.
