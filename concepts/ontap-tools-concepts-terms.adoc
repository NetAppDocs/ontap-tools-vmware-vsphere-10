...
Local storage backends with SVM credentials or cluster credentials are added through the remote plugin user interface and are limited to a vCenter. When added locally as cluster credentials, the associated SVMs of the cluster implicitly map with the vCenter for managing vVols or VMFS. ONTAP tools VMFS management, including SRA, supports SVM credentials without requiring a global cluster for VMFS-only management.
