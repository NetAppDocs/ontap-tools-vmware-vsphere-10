...
summary: "Review vCenter Server RBAC considerations for ONTAP tools before using it in a production environment."
---

= How vCenter Server RBAC works with ONTAP tools for VMware vSphere
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
ONTAP tools for VMware vSphere implements role-based access control (RBAC) with vCenter Server to control access to vSphere objects and administrative tasks. Understanding the RBAC model — including predefined roles, permission assignments, and how vCenter and ONTAP privileges interact — helps you configure appropriate access before deploying ONTAP tools in a production environment.
...
To view the ONTAP tools for VMware vSphere roles, select *Menu* at the top of the vSphere Client, select *Administration*, and then select *Roles* on the left.
The following privileges must be included in the role assigned to the vCenter user responsible for deploying or onboarding vCenter Server. Verify that these privileges are configured before starting the deployment or onboarding process.
...
ONTAP tools for VMware vSphere includes three predefined roles.
...
The two RBAC environments work together. When you perform a task in the vSphere Client, ONTAP tools checks the vCenter Server roles first. If the operation is permitted by vSphere, then the ONTAP role privileges are examined. This second step is performed based on the ONTAP role assigned to the user when the storage backend was created and configured.

== Working with vCenter Server RBAC

Keep the following in mind when working with vCenter Server privileges and permissions.

=== Required privileges

To access the ONTAP tools for VMware vSphere 10 user interface, you need to have the ONTAP tools-specific _View_ privilege. If you sign in to vSphere without this privilege and select the NetApp icon, ONTAP tools for VMware vSphere displays an error message and prevents you from accessing the user interface.
...
You can instead assign the View privilege to another lower vSphere object level. However, this limits the ONTAP tools for VMware vSphere menus that you can access.
