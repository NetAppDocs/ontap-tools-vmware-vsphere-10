...
summary: "Configure user roles and privileges for storage backends with the JSON file from ONTAP tools and ONTAP System Manager."
---
...
[.lead]
Configure ONTAP user roles and privileges for storage backends by using ONTAP tools for VMware vSphere and ONTAP System Manager. You can assign roles with the provided JSON files, manually create users and roles, or apply the minimum required privileges for non-admin accounts.

.Before you begin

* Download the ONTAP Privileges file from ONTAP tools for VMware vSphere at _\https://<ONTAPtoolsIP>:8443/virtualization/user-privileges/users_roles.zip_. The zip file contains two JSON files. Use the ASA r2-specific JSON file when you configure an ASA r2 system.
+
[NOTE]
...
.. Select the product capability as *VSC, VASA Provider, and SRA* from the drop-down list.
