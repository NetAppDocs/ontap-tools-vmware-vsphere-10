...
. Login to cluster with admin privileges. To configure cluster ONTAP tools or SVM ONTAP tools user, perform the following steps: 
...
. Access ONTAP System Manager using the cluster management IP address of the cluster.
. Login to cluster with admin privileges. To configure cluster ONTAP tools or SVM ONTAP tools roles, select *Cluster* > *Settings* > *Users and Roles* pane.
. Create Roles:
