...
* If you have enabled VASA Provider and you want to specify storage capability profiles for your NFS datastores or VMFS datastores, then you should have created one or more storage capability profiles.
...
|===
NOTE: A FlexGroup that is part of a NFS and VMFS datastore cannot shrink below the existing size but can grow by 120% maximum. Default snapshots are enabled on these FlexGroup volumes.
