...
sidebar: sidebar
keywords:
summary: 'VASA Provider for ONTAP enables you to automatically generate storage capability profiles for existing NFS and VMFS datastores. When you select the auto-generate option for a datastore, VASA Provider creates a profile that contains the storage capabilities that are used by that datastore.'
---
...
[.lead]
VASA Provider for ONTAP enables you to automatically generate storage capability profiles for existing NFS and VMFS datastores. When you select the auto-generate option for a datastore, VASA Provider creates a profile that contains the storage capabilities that are used by that datastore.
