...
. If you select *Custom export policy* under the *Advanced options*, run discovery in vCenter for all objects. It’s recommended that you don't use this option.
[NOTE]
...
* When you choose the *Any* option in the platform drop-down, you can see all SVMs in the vCenter. Platform and asymmetric flag do not affect visibility. 
...
* You can create a VMFS datastore only on an ESXi host protected by an asynchronous relationship. You cannot create and mount a datastore on an ESXi host that is part of a host cluster protected by an Automated Failover Duplex policy.
 
.Before you begin

* Enable services and LIFs for each protocol on the ONTAP storage side. 
* Map the SVM to an aggregate for the SVM user in the ASA r2 storage system. 
// https://jira.ngage.netapp.com/browse/OTVDOC-189 updates jani
