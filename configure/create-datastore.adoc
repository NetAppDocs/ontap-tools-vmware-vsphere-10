...
summary: "Create NFS, VMFS, or vVols datastores in ONTAP tools at the host or host cluster level."
---
...
[.lead]
Create an NFS, VMFS, or vVols datastore at the host or host cluster level. When you create a datastore at the host cluster level, ONTAP tools mounts it on all destination hosts, provided you have the required privileges.
...
--
Beginning with ONTAP tools for VMware vSphere 10.3, you can create a vVols datastore on ASA r2 systems with thin space efficiency. The VASA Provider creates a container and the desired protocol endpoints while creating the vVol datastore. The VASA Provider does not assign any backing volumes to this container. 

.Before you begin

* Make sure root aggregates are not mapped to the SVM. 
* Ensure that the VASA Provider is registered with the selected vCenter.
* For ASA r2 storage systems, SVM-to-aggregate mapping is not required.
...
+
* If you select the *Custom export policy*, run discovery in vCenter for all objects before you continue. NetApp recommends that you don't use this option.
* You can select a *Custom initiator group* name for the iSCSI and FC protocols.
+
[NOTE]
For an ASA r2 SVM, storage units (LUNs/namespaces) are not created because the datastore is only a logical container.
...
--
An NFS datastore connects ESXi hosts to shared storage by using the NFS protocol. It is simple and flexible, and VMware vSphere environments commonly use it.
...
.. Select *Datastore cluster* and *Kerberos authentication* in the *Advanced options*.
[NOTE]
Kerberos authentication is available only when the NFS 4.1 protocol is selected. 
+
.. Specify the *Number of connections (nconnect)* for the datastore. The default value is 4, configurable from 1 to 8. If the value you select is higher than the current host-level nconnect setting, ONTAP tools updates the host-level nconnect setting to match.
// 10.6 nconnect updates
. In the *Storage* pane, select *Platform* and *Storage VM*. 
. If you select *Custom export policy* under the *Advanced options*, run discovery in vCenter for all objects before you continue.
[NOTE]
You cannot create an NFS datastore using the SVM’s default or root volume policy.
[NOTE]
* In the *Advanced options*, the *Asymmetric* toggle button is visible only if you select performance or capacity in the platform drop-down list. However, the *Asymmetric* option doesn't apply to NFS datastore creation.
* When you choose the *Any* option in the platform drop-down, all SVMs in the vCenter are visible. Platform and asymmetric settings do not affect visibility.
. In the *Storage Attributes* pane, select the aggregate for volume creation. In the *Advanced options*, choose *Space Reserve* and *Enable QoS* as required.
. In the *Summary* pane, review the selections and select *Finish*.
...
. Check if the Network Interface Card (NIC) vendor supports ESXi NICs with the NVMe/TCP protocol. 
...
. Run the link:../configure/discover-storage-systems-and-hosts.html[Discover storage systems and hosts] action on the ESXi host.
...
. If not already enabled, enable NVMe over Fabrics (NVMe-oF) on your ESXi hosts. 
...
For more information on using NVMe/FC with VMware vSphere 7.0, refer to https://docs.netapp.com/us-en/ontap-sanhost/nvme-esxi-7.html[ONTAP NVMe/FC Host Configuration guide].
...
[NOTE]
For ASA r2 storage systems, the *Aggregate* option is not shown because storage is disaggregated. When you choose an ASA r2 storage system type SVM, the *Storage attributes* page shows options for enabling QoS.
...
.. Select the *Performance service level* for an ASA r2 storage system with a 12-node SVM in a heterogeneous cluster. This option is unavailable if the selected SVM is a homogeneous cluster or uses an SVM user.
+
...
. In the *Protection* window, select *Protect this datastore using SnapMirror replication* to enable protection. You can add the datastore to an existing protection group or create a new one. To create a new protection group:
.. Enter a *Protection group name*.
.. Configure the SnapMirror relationship. If you select the Automated Failover Duplex policy(AFD), set up host cluster configuration settings before continuing. For details, see link:../protect/config-proximity.html[Configure cluster for datastore protection with ONTAP tools].
[NOTE]
If the selected volume is already part of another protection setting, the next protection page is disabled and an informational message appears.
If the datastore is not eligible for protection, the protection option is disabled and the reason for ineligibility is displayed. You can still create the datastore without protection.
. Review the datastore details in the *Summary* pane and select *Finish*.
// 10.6 updates 
[NOTE]
If a datastore is protected, ONTAP tools mounts it on all hosts in the cluster.
