...
. In the *Protection* window, select *Protect this datastore using SnapMirror replication* to enable protection. 
. If the datastore is eligible for protection, you can choose to add the datastore to an existing protection group or create a new one. To create a new protection group:
.. Enter a *Protection group name*. 
.. Add SnapMirror relationship.  If you select the AutomatedFailoverDuplex policy, configure host proximity and access settings before proceeding. For more information, see link:../protect/config-proximity.html[Configure proximity and access for datastore protection with ONTAP tools].
