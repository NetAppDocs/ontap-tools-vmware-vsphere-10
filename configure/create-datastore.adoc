...
+
* If you select the *Custom export policy*, run discovery in vCenter for all objects before you continue. It’s recommended that you don’t use this option.
...
--
An NFS datastore connects ESXi hosts to shared storage using the NFS protocol. It is simple and flexible and is used in VMware vSphere environments.
...
[NOTE]
* In the *Advanced options*, the *Asymmetric* toggle button is visible only if performance or capacity is selected in the platform drop-down. However, the *Asymmetric* option does not apply to NFS datastore creation.
