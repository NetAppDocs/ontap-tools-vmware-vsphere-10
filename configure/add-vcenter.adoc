...
summary: "Add vCenter Server instances to ONTAP tools to configure, manage, and protect datastores."
---
...
[.lead]
Add vCenter Server instances to ONTAP tools for VMware vSphere to configure, manage, and protect datastores in your vCenter Server environment. When you add multiple vCenter Server instances, configure Custom CA certificates for secure communication between ONTAP tools and each vCenter Server.
...
* ONTAP tools applies custom privileges for the plug-ins and APIs to the vCenter Server instance.
* ONTAP tools creates custom roles to manage the users.
...
* Deploy vCenter Server using its fully qualified domain name (FQDN) and ensure the SAN in the certificate includes DNS Name=machine_FQDN for optimal compatibility and support.
...
[NOTE]
====
If FQDN is not available, you can set the PNID to the IP address and include the IP address in the SAN. However, VMware does not recommend this configuration.
====
