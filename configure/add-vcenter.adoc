...
.Before you begin
// OTVDOC-347 updates
* Broadcom recommends deploying vCenter Server using its fully qualified domain name (FQDN), and ensuring the Subject Alternative Name (SAN) in the certificate includes DNS Name=machine_FQDN for optimal compatibility and support.
* Ensure that the vCenter Server certificate includes a valid Subject Alternative Name (SAN) extension with both DNS and IP address entries. The SAN should match how clients connect to vCenter (FQDN and/or IP address).
* The Primary Network Identifier (PNID) of the vCenter Server must be included in the SAN details. PNID and DNS name should be identical and resolvable in DNS.
