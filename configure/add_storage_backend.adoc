...
This task helps you to onboard the ONTAP cluster. When you add storage backend using ONTAP tools Manager UI, the storage backend is added to the global cluster. Associate the global cluster with vCenter to enable direct SVM for vVol provisioning.
