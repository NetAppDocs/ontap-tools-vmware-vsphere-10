...
[NOTE]
For ASA r2 storage systems, the *Aggregate* option is not shown as the ASA r2 storage is a disaggregated storage. When you choose ASA r2 type SVM, the storage attributes page shows the options to enable QoS.
...
.. The *Performance service level* option is available when the selected storage VM is an ASA r2 storage system with a 12-node SVM of type heterogeneous cluster. 'Any' is the default performance service level value. This setting creates the storage unit using the ONTAP balanced placement algorithm. However, you can select the performance or extreme option as required.
+
The performance service level input field is not available if you have selected an SVM that is a homogeneous cluster or uses an SVM user.
// updated for 10.4
