...
sidebar: sidebar
keywords:
summary: 'You can use ONTAP tools to manually create or edit storage capability profiles, automatically generate a profile based on the capabilities of a datastore, or modify a profile to meet your requirements.'
---
= Create or edit storage capability profiles
:icons: font
:imagesdir: ../media/

[.lead]
You can use ONTAP tools to manually create or edit storage capability profiles, automatically generate a profile based on the capabilities of a datastore, or modify a profile to meet your requirements.
...
. On ONTAP tools Home page, click *Policies and Profiles*.
...
If you have enabled "`linked`" mode for your vCenter Servers, then you should select the vCenter Server for which you are creating the storage capability profile.
a|
Platform
a|
You can create or edit storage capability profiles using the combination of the following items:
...
+
You can set the value for *Max IOPS* and *Min IOPS* which enables you to use the QoS functionality. If you select Infinite IOPS, the Max IOPS field is disabled. When applied for a NFS and VMFS datastore, a QoS policy with "`Max IOPS`" value is created and assigned to a FlexVol volume. When used with a vVols datastore, a QoS policy with Max IOPS and Min IOPS values is created for each data vVols datastore.
+
*NOTE*:

  *** Max IOPS and Min IOPS can also be applied to the FlexVol volume for a NFS and VMFS datastore.
  *** You should ensure that the performance metrics are not also set separately at an storage virtual machine (SVM) level, an aggregate level, or a FlexVol volume level.
 ...
 ** Snapshot: Moves user data blocks of volume Snapshots that are not associated with the active file system to the capacity tier
