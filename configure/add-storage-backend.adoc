...
|Adding an entire storage cluster as a storage backend for a single vCenter server configuration
|Onboard a full ONTAP cluster as a backend for one vCenter
|Adding a storage backend from the ONTAP tools plugin in vCenter
|No need to associate the storage backend with the vCenter in ONTAP tools Manager for this case

|Adding multiple storage virtual machines (SVMs) in a multi-tenancy environment and associating them to different vCenter server instances
|Onboard SVMs as backends and associate them with different vCenters in a multi-tenant setup
|Adding a storage virtual machine as a storage backend
|Include when associating a storage backend/SVM with a particular vCenter is required

|Comparison of Global and Local Storage Use Cases
|Global: Multi-tenancy where an ONTAP tools admin manages multiple vCenters and assigns SVM-level access to each vCenter. Local: Onboarding a cluster or directly onboarding an SVM for a specific vCenter
|--
|--

|vCenter Storage Backend Onboarding
|Onboarding a storage backend with SVM in vCenter requires both cluster onboarding (with limited privileges) and SVM onboarding at the vCenter level, especially for vWAL workflows
|--
|Clarifies onboarding procedures for clusters and SVMs in both single and multi-vCenter environments, and the distinction between global and local storage backends
|===
