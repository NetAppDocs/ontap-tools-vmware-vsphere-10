...
In a multitenancy setup where vCenter acts as the tenant with an associated SVM, use ONTAP tools Manager to add the cluster. Associate the storage backend with the vCenter Server to map the storage backend globally. First, onboard the desired Storage Virtual Machines (SVMs) in vCenter tenant. This allows the SVM user to provision vVols datastores. You can then add storage in vCenter using the SVM.
...
// 10.5 updates -Jani rewrote the intro and added the last para.
