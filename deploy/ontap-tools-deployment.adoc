...
image:../media/custom-temp.png[Customize template]
[NOTE]
The vCenter hostname is the name of the vCenter Server instance where the ONTAP tools appliance is deployed.
+
If you are deploying ONTAP tools in a two-vCenter topology—where the appliance is hosted in one vCenter instance and manages another—you can assign a restricted role for the vCenter instance hosting ONTAP tools.
Create a dedicated vCenter user and role with only the datastore permissions required for OVF template deployment, as listed below:
...
Set up a vCenter user account for the vCenter environment where the ONTAP tools appliance will be deployed. Assign the following datastore permissions to this account:
...
For more information, refer to https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/9-0/vsphere-security/vsphere-permissions-and-user-management-tasks/using-roles-to-assign-privileges.html[Using vCenter Roles to Assign Privileges].
