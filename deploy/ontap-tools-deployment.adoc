...
[.lead]
When you deploy ONTAP tools for VMware vSphere for the first time it is deployed in a single node, small node size configuration with traditional node capabilities.
