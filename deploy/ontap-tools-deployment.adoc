...
---
= Deploy ONTAP tools as a single-node appliance
:icons: font
:imagesdir: ../media/
...
ONTAP tools for VMware vSphere is initially deployed as a non-HA configuration. Beginning with ONTAP tools 10.6, CPU hot add and memory hot plug are enabled by default. To scale to HA later, these features must be enabled.
