...
Make sure the network route is present and the storage data network is accessible from VM management network.
...
You need to create a content library to store the OVA before changing the appliance settings to HA configuration.
[NOTE]
