...
|===
. In the *Select a name and folder* field, enter the virtual machine name and choose its location. To enable the ability to use the HA feature, check the *Customize this virtual machine's hardware* checkbox, an additional step called *Customize hardware* will be enabled before reaching the *Ready to complete* window.
...
. In the *Customize hardware* window, enable CPU hot add and memory hot plugin to enable the ability to use HA.
