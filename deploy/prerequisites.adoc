...
The recommended memory for each deployment size is shown in the table below. For high availability (HA) deployments, you need three times the appliance size listed.
...
* Internal ports: These ports are intended for communication strictly within the Kubernetes cluster, usually bound to ClusterIP addresses. They support interactions between internal services and are not exposed to external networks by default. To ensure security, configure firewall rules to prevent external access, even in case of misconfiguration. These ports are needed for internal communication. No need to add these ports in firewall rules.
...
* Inter-node ports: These ports enable communication between nodes within the Kubernetes cluster. They are essential for internal cluster operations, such as data exchange and coordination. For single-node deployments, inter-node ports are used only within the node and do not require external access.
...
|rpcbind/portmapper |111 |TCP/UDP |Internal |Used for RPC communication between services.
|coredns (DNS) |53 |TCP/UDP |Internal |Domain Name System (DNS) service for name resolution within the cluster.
|NTP |123 |UDP |Internal |Network Time Protocol (NTP) for time synchronization.
|etcd |2379, 2380, 2381 |TCP |Internal |Key-value store for cluster data.
|ntv-gateway-svc |30000-32767 |TCP |NodePort |NodePort port range for cluster internal connections.
|kube-vip |2112 |TCP |Internal |Kubernetes API server port.
|kubelet |10248, 10250 |TCP |Internal |	Kubernetes component
|kube-controller |10257 |TCP |Internal |Kubernetes component
|cloud-controller |10258 |TCP |Internal |Kubernetes component
|kube-scheduler |10259 |TCP |Internal |Kubernetes component
|kube-proxy |10249, 10256 |TCP |Internal |Kubernetes component
|calico-node |9091, 9099 |TCP |Internal |Calico networking component.
|kube-apiserver |6443 |TCP |External |Kubernetes API server port (Restrict to trusted networks).
|rke2 server |9345 |TCP |External |RKE2 supervisor API (Restrict to trusted networks).
|containerd |10010 |TCP |Internal |Container daemon service.
|VXLAN (Flannel) |8472 |UDP |Internal|Overlay network for pod communication.
...
// |123|TCP |Ntpd uses this port to perform validation of the NTP server. |3300 and 6789-7300 |TCP |Inter-node data plane 
