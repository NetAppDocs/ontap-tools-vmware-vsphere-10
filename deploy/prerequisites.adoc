...
[NOTE]
The total number of ESXi hosts and datastores is what matters for ONTAP tools configuration, regardless of whether they are managed by a single vCenter or multiple vCenters.

[NOTE]
ONTAP tools runs a parallel discovery of all storage objects, so ONTAP storage object configuration limits apply even if only a few objects are in use.
