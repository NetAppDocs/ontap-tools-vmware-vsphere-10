...
[cols="2,2,2,1,2,1"]
|===
|Deployment |Number of VMFS and NFS datastores |Number of DR enabled VMFS datastores |Number of hosts | Number of vCenters |ONTAP Storage objects

|Non-HA Small |200 |80 |32 |1 or multiple |
|Non-HA Medium |250 |100 |32 |1 or multiple |
|HA Small |350 |200 |64 |1 or multiple |
|HA Medium |600 |200 |128 |1 or multiple |
|HA Large |1024 |250 |256 |1 or multiple |
|===

Disaster Recovery (DR) configuration is done through SnapMirror Active Sync, and multi-vCenter deployments are not supported with SnapMirror Active Sync.

[NOTE]
ONTAP tools only restricts the number of ESXi hosts and datastores based on the deployment size. There are no restrictions on the number of vCenter Servers that can be connected to ONTAP tools.

[NOTE]
ONTAP tools performs parallel discovery of all storage objects. The configuration limits for ONTAP storage objects apply regardless of the number of objects actively in use.
