...
sidebar: sidebar
keywords: requirements, configuration limits, supported versions, sizing, port requirements, ONTAP tools for VMware vSphere
summary: 'Review supported versions, sizing, port requirements, and configuration limits before deploying ONTAP tools for VMware vSphere.'
---
...
[.lead]
Review supported versions, sizing, port requirements, and configuration limits before deploying ONTAP tools for VMware vSphere.

Deploy ONTAP tools for VMware vSphere on a supported vSphere environment that includes ESXi hosts and VMware vCenter Server Virtual Appliance (vCSA).
...
The table below shows the recommended CPU, memory, and disk space per node for each deployment size. For high availability (HA) deployments, deploy three nodes.
// updated for GITHUB issue #50
|===
|*Type of deployment*| *CPUs per node* |*Memory (GB) per node* |*Disk space (GB) thick provisioned per node*
|Extra small (XS) |6 |12 |350
// 10.6 XS updates
|Small |9 |18 |350
|Medium |13 |26 |350
|Large |17 |34 |350
|===
[NOTE]
Large deployment size is supported only for HA configuration.
[NOTE]
When backup is enabled, each ONTAP tools cluster needs an additional 50 GB of space on the datastore where VMs are deployed. Therefore, non-HA requires 400 GB, and HA requires 1100 GB of space in total.
...
|ESXi hosts managed by ONTAP tools
|7.0.3 onwards

|vCenter Server instances managed by ONTAP tools
|8.0U3 onwards
...
|ESXi host used to deploy the ONTAP tools virtual machine
|7.0U3 and 8.0U3
//OTVDOC-282 updates -Jani

|vCenter Server used to deploy the ONTAP tools virtual machine
|7.0 and 8.0
// updated the requirements as per OTVDOC-258
|===
[NOTE]
Beginning with ONTAP tools for VMware vSphere 10.4, the virtual machine hardware changes from version 10 to 17.
...
* Inter-node ports: These ports enable communication between nodes within the Kubernetes cluster. They are required for cluster operations such as data exchange and node coordination. For single-node deployments, inter-node ports are used only within the node and do not require external access. Allow inter-node traffic only from trusted cluster networks, and block internet access with firewall rules.
...
Use the following table as a guide for configuring ONTAP tools for VMware vSphere.
|===
...
|HA |Small (S) |up to 24K| 64
|HA |Medium (M) |up to 50K| 128
|HA |Large (L) |up to 100K| 256 
|===
