...
The table below shows the recommended CPU, memory, and disk space for each deployment size. For high availability (HA) deployments, you need three nodes (totals shown in parentheses).
// updated for GITHUB issue #50
|===
|*Type of deployment*| *CPUs per node* |*Memory (GB) per node* |*Disk space (GB) thick provisioned per node*
|Non-HA Extra small (XS) |6 |12 |350
|HA Extra small (XS) |6 (18 total) |12 (36 total) |350 (1050 total)
// 10.6 XS updates
|Small |9 |18 |350
|Medium |13 |26 |350
|Large |17 |34 |350
|===
NOTE: The large deployment is only for HA configuration. For HA deployments, the values in parentheses indicate the total across all three nodes.
[NOTE]
