...
Host names should be mapped to the free IP addresses on the DNS before assigning for both HA and non-HA deployments. All the five IP addresses in HA deployment and the three 3 IP addresses in non-HA deployment should be on the same VLAN that is selected for deployment. 
