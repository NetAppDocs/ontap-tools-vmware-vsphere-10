...
+
Advanced deployment option uses ONTAP trident instead of trident as storage backend and the easy deployment uses local storage.  In advanced deployment, as trident is not the storage backend, ONTAP becomes the storage backend.
