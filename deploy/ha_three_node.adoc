...
[NOTE]
Creating the content library is a mandatory step for deploying HA three nodes configuration. See link:../deploy/concept_how_to_download_ontap_tools.html[Download ONTAP tools] for details. Learn more https://docs.vmware.com/en/VMware-vSphere/8.0/vsphere-vm-administration/GUID-A58AF4FD-6CBE-4210-9E67-27EFBDCC1EF2.html[Create and Edit a Content Library].
...
. If you have not imported the OVA file yet, navigate to your content library and select *Actions* in the right side of the page and select *Import item* to import the OVA file.
. Navigate to the resource pool or the host where you want to deploy the OVA.
. Right-click the required location where you want to deploy the VM, and select *Deploy OVF template...*.
...
.. Enter the Load Balance IP details. This is the IP that is assigned for VASA.
