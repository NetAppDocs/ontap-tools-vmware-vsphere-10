...
* Small HA three node contains 12 CPUs and 24 GB RAM.
* Medium HA three node contains 24 CPUs and 48 GB RAM.
* Large HA three node contains 36 CPUs and 72 GB RAM.
...
. In *Customize template* > *Content Library Details* window, enter the *Content Library Name* and the *OVF Templete Name*.
...
.. Secondary DNS 
.. Search Domains 
. In *Customize template* > *Node2 Configuration* and *Node3 Configuration* window, enter the following details:
.. Datacenter
.. Network vLAN name
.. Folder path
.. Resource pool
.. Cluster
.. Exsi host - This can be the same as the previous node or can be a different ESXi host.
.. Datastore
.. HostName
.. IP Address
.. Prefix length (only for IPV6)
