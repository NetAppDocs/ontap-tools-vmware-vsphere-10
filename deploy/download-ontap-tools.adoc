...
When the deployment is complete, ONTAP tools for VMware vSphere and VASA products are installed in your environment. By default, ONTAP tools for VMware vSphere starts working as soon as you decide on the subsequent deployment model and choose whether to enable VASA Provider based on your requirements. See link:../configure/registration-process.html[Register the VASA Provider with a vCenter Server instance] for details.
