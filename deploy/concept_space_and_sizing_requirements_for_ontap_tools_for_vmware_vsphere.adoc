...
Specify a valid DNS hostname (unqualified) as well as the static IP address for ONTAP tools and the other network parameters. The IP addresses provided should be accessible from the VLAN network that you select during the deployment. DHCP is not supported for ONTAP tools for VMware vSphere 10.1 release. All of these parameters are required for proper installation and operation.
