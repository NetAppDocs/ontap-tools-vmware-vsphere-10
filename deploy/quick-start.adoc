...
[.lead]
Use this quick start to deploy ONTAP tools for VMware vSphere and complete core setup tasks, including vCenter onboarding, role configuration, storage backend setup, and optional protection features.

Initially, you’ll deploy ONTAP tools for VMware vSphere as an Extra small (XS) single-node configuration that provides core services to support NFS and VMFS datastores. After the initial deployment, you can expand your configuration, refer to the link:../deploy/ha-workflow.html[HA deployment workflow] to learn more.
...
[role="quick-margin-para"]
Scale up to small (S), medium (M), or large (L) configurations when enabling additional services or higher scale is needed. 
...
[role="quick-margin-para"]
Configure ONTAP tools for VMware vSphere to manage granular datastore protection. Perform the ONTAP cluster and SVM peering in ONTAP systems to use SnapMirror active sync. This applies only to VMFS datastores.
...
[role="quick-margin-para"]
Backups are enabled by default in ONTAP tools for VMware vSphere 10.6 and run every 10 minutes. Use these backups to recover your ONTAP tools deployment after a failure.
