...
[role="quick-margin-para"]
Initially, you’ll deploy ONTAP tools for VMware vSphere as an Extra small (XS) single-node configuration (the default initial setup) that provides core services to support NFS and VMFS datastores. Scale up to small (S), medium (M), or large (L) configurations when enabling additional services or higher scale is needed.
