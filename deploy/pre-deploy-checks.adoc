...
summary: "Complete the pre-deployment checklist for ONTAP tools, including vSphere compatibility, sizing, certificates, and network requirements."
---
...
|ONTAP tools IP address (formerly load balancer). This external IP address is used to access the ONTAP tools Manager interface at `\https://<ip>:8443/virtualization/ui/`, send API calls, and access the ONTAP tools plug-in from the vCenter interface.
// updated for https://jira.ngage.netapp.com/browse/OTVDOC-367
|
|ONTAP tools virtual IP address (formerly node interconnect). This IP address is assigned to the Kubernetes API server and acts as the virtual IP address for the Kubernetes control plane in single-node and multi-node deployments. It is used for internal cluster communication and Kubernetes management, and it must be different from the ONTAP tools IP address.
// updated for https://jira.ngage.netapp.com/browse/OTVDOC-367 
|
...
|DNS search domain|
|IPv4 address for the ONTAP tools management node. In a single-node deployment, this address maps to the specified host name for the ONTAP tools VM. In an HA deployment, you must provide additional IPv4 addresses for the two additional ONTAP tools VMs.
// updated for https://jira.ngage.netapp.com/browse/OTVDOC-367
|
