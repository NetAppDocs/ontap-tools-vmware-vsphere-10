...
Determine if you will use multi-tenant SVMs or allow full cluster access. If using multi-tenant SVMs, you will need an SVM management LIF on each SVM.
...
|IP address for the internal services|
|DNS hostname for the first node|
|IP address for the first node|
|Subnet mask for the IP addresses|
|Default gateway for the IP addresses|
|Primary DNS server|
