...
summary: "Use ONTAP tools deployment error codes to diagnose and resolve deployment, reboot, and recovery failures."
---
= ONTAP tools deployment error codes
:icons: font
:imagesdir: ../media/

[.lead]
If you experience deployment issues, review the logs and error codes to diagnose and resolve problems. Beginning with ONTAP tools for VMware vSphere 10.6, log bundles collected from the pods include logs from MongoDB, RabbitMQ, and Vault, along with the status and descriptions of all pods. ONTAP tools 10.6 also adds audit logging to improve traceability during troubleshooting. These are provided in addition to the existing ONTAP tools service logs, enhancing supportability and troubleshooting.
...
All error logs are recorded in the ansible-perl-errors.log file under the /var/log/otvng directory to facilitate easy tracking and resolution of issues. This log file contains the error code and the failed Ansible task.
[NOTE]
...
|061 |Services deployment has failed |Perform basic Kubernetes debugging like get pods, get rs, get svc, and so on in ntv-system namespace for more details and error logs at /var/log/otvng/ansible-perl-errors.log and /var/log/otvng/ansible-run.log and redeploy.
|062 |ONTAP tools Services deployment has failed |Refer to the error logs at /var/log/otvng/ansible-perl-errors.log for more details and redeploy.
