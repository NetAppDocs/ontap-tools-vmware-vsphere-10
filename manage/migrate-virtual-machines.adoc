...
For information on migrating virtual machines, refer to the Broadcom VMware documentation
...
You can take crash-consistent snapshots (snapshots without memory state, which can be used to restart the virtual machine to a consistent state) on all supported datastore types. Quiesced snapshots, which use VMware Tools to flush in-memory data to disk before capturing, are also supported. For an overview of snapshot types and behavior, refer to the Broadcom VMware documentation.
...
For information on managing virtual machine snapshots, refer to the Broadcom VMware documentation.
