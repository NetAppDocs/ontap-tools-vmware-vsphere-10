...
[.lead]
You can recover the ONTAP tools for VMware vSphere setup by restoring it from a backup file stored on the datastore. Beginning with ONTAP tools for VMware vSphere 10.5, the backup feature is enabled by default, so a recent backup is available if you need to redeploy or recover the appliance.
