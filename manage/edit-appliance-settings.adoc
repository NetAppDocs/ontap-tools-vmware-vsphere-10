...
[NOTE]
In ONTAP tools for VMware vSphere, you're only allowed to increase the node size; you cannot reduce the node size. The default deployment is Extra Small (XS). In a non-HA setup, Small (S) and Medium (M) configurations are supported. In an HA setup, Small (S), Medium (M), and Large (L) configurations are supported.
+
. Use the HA toggle button to enable the HA configuration. On the *HA settings* page, ensure that:
