...
+
[NOTE]
On extra small (XS) deployments, certain services and features are unavailable or appear disabled. To enable these services, increase the appliance size to at least Small (S) or the minimum supported size required for the feature.
// 10.6 XS updates
. Use the HA toggle button to enable the HA configuration. On the *HA settings* page, ensure that:
[NOTE]
In ONTAP tools for VMware vSphere, you're only allowed to increase the node size; you cannot reduce the node size. The default deployment is Extra Small (XS). In a non-HA setup, Small (S) and Medium (M) configurations are supported. In an HA setup, Extra Small (XS), Small (S), Medium (M), and Large (L) configurations are supported.
+
. Use the HA toggle button to enable the HA configuration. On the *HA settings* page, ensure that:
