...
Beginning with ONTAP tools for VMware vSphere 10.4, ONTAP tools automatically creates and maintains ONTAP and vCenter objects, simplifying datastore management in VMware datacenter environments.
...
[NOTE]
Only custom igroups can be reused. Igroups created during migration from ONTAP tools 9.x (migrated igroups) cannot be reused and, beginning with ONTAP tools 10.5P2, are automatically deleted when no longer in use.
// OTV10.5 P2 updates
