---
permalink: manage/manage-vcenter.html
sidebar: sidebar
keywords:
summary: 'vCenters are central management platforms that allow you to control hosts, virtual machines, and storage backends.'
---
= Manage vCenter Server instances in ONTAP tools
:icons: font
:imagesdir: ../media/

[.lead]
vCenter Server instances are central management platforms that allow you to control hosts, virtual machines, and storage backends.

== Dissociate storage backends with the vCenter Server instance

The vCenter Server listing page shows the associated number of storage backends. Each vCenter Server instance has the option to associate or disassociate with a storage backend.

.Steps

. Launch ONTAP tools Manager from a web browser: `\https://<ONTAPtoolsIP>:8443/virtualization/ui/` 
. Log in with the ONTAP tools for VMware vSphere administrator credentials you provided during deployment. 
. Select the required vCenter Server instance from the sidebar.
. Select the vertical ellipses against the vCenter Server that you want to associate or dissociate with storage backends.
. Select *Dissociate storage backend*.

== Modify a vCenter Server instance
Follow the steps below to modify a vCenter Server instances.

. Launch ONTAP tools Manager from a web browser: `\https://<ONTAPtoolsIP>:8443/virtualization/ui/` 
. Log in with the ONTAP tools for VMware vSphere administrator credentials you provided during deployment. 
. Select the applicable vCenter Server instance from the sidebar
. Select the vertical ellipses against the vCenter Server that you want to modify and select *Modify*.
. In the *Modify vCenter* window, enter the username, password, and port details. 
. Upload the certificate  and select *Modify*.


== Remove a vCenter Server instance
Remove all storage backends from the vCenter Server before removing it.

. Launch ONTAP tools Manager from a web browser: `\https://<ONTAPtoolsIP>:8443/virtualization/ui/` 
. Log in with the ONTAP tools for VMware vSphere administrator credentials you provided during deployment. 
. Select the applicable vCenter Server instances from the sidebar
. Select the vertical ellipses against the vCenter Server that you want to remove and select *Remove*.
+
[NOTE]
After you remove vCenter Server instances, they will no longer be maintained by the application.

When you remove vCenter Server instances in ONTAP tools, the following actions are performed automatically: 

* Plug-in is unregistered.
* Plug-in privileges and plug-in roles are removed.

=== Force remove a vCenter Server instance
// 10.6 updates
Beginning with ONTAP tools 10.6, you can use the force delete option to remove a vCenter Server instance even when VMFS and NFS datastores, and storage backends are still associated.

Use force delete when the vCenter Server is decommissioned, unreachable, or no longer needed in ONTAP tools.

[NOTE]
Force delete removes only ONTAP tools metadata. It does not remove ONTAP storage objects (volumes or LUNs), vSphere datastores, or virtual machines.

The *Remove vCenter* dialog behavior depends on the state of the vCenter Server:

* *vVols datastores or protected datastores exist*: Removal is blocked. You cannot remove the vCenter Server until you remove or migrate all vVols datastores and unprotect all protected datastores.

* *No vVols or protected datastores, but storage backends are associated*: The dialog displays the number of associated storage backends and datastores. You must select both of the following options before you can proceed:
** *Remove all storage artifacts from this vCenter Server*
** *I understand that the associated datastores will no longer be managed by ONTAP tools*

* *No vVols datastores, protected datastores, or storage backends*: ONTAP tools removes the vCenter Server. The application will no longer manage it.

[NOTE]
Global storage backends shared with other vCenter Server instances are retained. Only the association with the deleted vCenter Server is removed.

.Steps

. Launch ONTAP tools Manager from a web browser: `\https://<ONTAPtoolsIP>:8443/virtualization/ui/`
. Log in with ONTAP tools for VMware vSphere administrator credentials.
. Select *vCenters* from the sidebar.
. Select the vertical ellipses for the required vCenter Server and select *Remove*.
. In the *Remove vCenter* dialog, review the state of the vCenter Server.
.. If the dialog displays a blocked message, resolve the vVols or protected datastore conditions before retrying.
.. If the dialog lists associated storage backends and datastores, select *Remove all storage artifacts from this vCenter Server* and the acknowledgment checkbox.
.. If no blockers or storage backends are listed, proceed to the next step.
. Select *Force delete*.
. Confirm the action.
+
[NOTE]
If the vCenter Server is unreachable, plug-in unregistration is not performed and a warning is displayed.

== Renew vCenter Server certificate
// OTVDOC-342 updates
ONTAP tools notifies you when the vCenter certificate is nearing expiration or has expired. After renewing the vCenter certificate, upload the new certificate to ONTAP tools using the following steps:

. Log in to the ONTAP tools remote diagnostics shell.
. Obtain the renewed vCenter certificate from the diagnostics shell:
+
----
echo | openssl s_client connect <vcenter>:443 2>&1 | sed -n '/-BEGIN CERTIFICATE/,/END CERTIFICATE/p' 
----
. Ensure the certificate is in Base 64 ASCII format and includes the beginning and ending lines, for example:
+
----
---{}BEGIN CERTIFICATE{}---

---{}END CERTIFICATE{}---
----
. Copy the output and save it as a text file with a `.pem` extension on your desktop.
. Launch ONTAP tools Manager from a web browser: `\https://<ONTAPtoolsIP>:8443/virtualization/ui/` 
. Log in with the ONTAP tools for VMware vSphere administrator credentials you provided during deployment.
. Select the applicable vCenter Server instance from the sidebar
. Select the vertical ellipses against the vCenter Server that you want to modify and select *Modify*.
. In the *Modify vCenter* window, enter the username, password, and port details. 
. Upload the certificate  and select *Modify*.

.Related information

link:../manage/configure-remote-diagnostic-access.html[Configure remote diagnostic access]