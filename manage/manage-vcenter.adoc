...
== Remove a vCenter Server instance using the force delete option

Beginning with ONTAP tools 10.6, you can use the force delete option to remove a vCenter Server instance even when traditional (non-vVol) datastores and storage backends are still associated.
...
Force delete is blocked if:

* A vVol datastore exists on the vCenter Server
* SMaS (granular datastore protection) is enabled on any datastore for the vCenter Server

If either condition exists, remove or migrate vVol datastores and unprotect protected datastores before proceeding.
...
=== Remove a reachable vCenter Server instance
...
. In the *Remove vCenter* dialog, review associated storage backends and datastores.
. Select *Force delete*.
. Confirm the action.

=== Remove an unreachable vCenter Server instance
...
. Log in with ONTAP tools for VMware vSphere administrator credentials.
. Select *vCenters* from the sidebar.
. Select the vertical ellipses for the unreachable vCenter Server and select *Remove*.
. In the *Remove vCenter* dialog, review associated storage backends and datastores.
. Select *Force delete*.
. Confirm the action.
