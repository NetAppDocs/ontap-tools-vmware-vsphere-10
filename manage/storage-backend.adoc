...
[.lead]
Storage backends are systems that ESXi hosts use for data storage. You can discover, modify, and remove storage backends using ONTAP tools for VMware vSphere.
...
The storage backend page lists all the storage backends. You can discover, modify, and remove the storage backends that you added, but you cannot perform these operations on the individual child SVM under the cluster. 
