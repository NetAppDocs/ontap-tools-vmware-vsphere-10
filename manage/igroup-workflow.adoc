...
summary: "Understand how ONTAP tools creates and manages igroups and export policies for datastore access."
---
= Igroup and export policy workflows in ONTAP tools
:icons: font
:imagesdir: ../media/

[.lead]
Use this information to understand how ONTAP tools for VMware vSphere creates and manages igroups and export policies for datastore access.
...
ONTAP tools for VMware vSphere 10.4 introduces nested igroups, where each datastore in vCenter is associated with a parent igroup, while each host is linked to a child igroup under that parent. You can define custom parent igroups with user-defined names for reuse across datastores to make igroup management easier.
Use this information to understand how ONTAP tools for VMware vSphere manages LUNs and datastores through igroup workflows.
...
ONTAP tools for VMware vSphere creates a child igroup host2Igroup and adds it to the existing parent igroup DS1Igroup.
...
When you delete datastores, ONTAP tools removes the host IP address from the export policy and assigns the default export policy if ONTAP requires access.  
