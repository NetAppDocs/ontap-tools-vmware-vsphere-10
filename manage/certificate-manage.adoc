...
summary: "Manage ONTAP tools and VASA Provider certificates, renew default certificates, or replace with custom CA certificates."
---
...
[.lead]
ONTAP tools and VASA Provider use a self-signed certificate by default. In ONTAP tools Manager, you can renew that certificate or replace it with a custom CA certificate. Multi-vCenter deployments require custom CA certificates.
...
[NOTE]
An ONTAP tools IP address should map to a fully qualified domain name (FQDN). Certificates should contain the same FQDN in the Subject Alternative Name (SAN) field.
...
--
The ONTAP tools tab shows details such as the certificate type (self-signed or CA-signed) and domain name.
During deployment, ONTAP tools generates a self-signed certificate by default. You can renew the certificate or upgrade it to a CA-signed certificate.
...
+
[NOTE]
The system is offline until the certificate renewal completes, and ONTAP tools Manager logs you out.
...
ONTAP tools for VMware vSphere uses a self-signed certificate for VASA Provider by default. With that certificate, you can manage only one vCenter Server instance for vVols datastores.
...
.. In the pop-up window, upload the server certificate, server certificate private key, root CA certificate, and intermediate certificate files.
.. Enter the FQDN mapped to the ONTAP tools IP address (formerly load balancer IP) for which you generated this certificate, and upgrade the certificate.
// updated for OTVDOC-341 and 367
+
[NOTE]
The system will be offline until the upgrade is complete, and you will be logged out of the ONTAP tools Manager interface. 

--
.Refresh ONTAP tools services certificate
--
...
. Enter `9` to select *Refresh ONTAP tools services certificates*.
